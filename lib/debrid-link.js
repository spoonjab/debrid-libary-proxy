...
async function getTorrentDetails(apiKey, ids, addonUrl) {
    const DL = new DebridLinkClient(apiKey)

    return await DL.seedbox.list(ids)
        .then(result => result.value)
        .then(torrents => torrents.map(torrent => toTorrentDetails(torrent, addonUrl)))
        .catch(err => handleError(err))
}
...
function toTorrentDetails(item, addonUrl) {
    const videos = item.files
        .filter(file => isVideo(file.name))
        .map(file => {
            const url = `${addonUrl}/resolve/DebridLink/null/${item.id}/${encode(file.downloadUrl)}`
            return {
                id: file.id,
                name: file.name,
                url: url,
