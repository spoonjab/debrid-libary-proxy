...
async function searchFiles(fileType, apiKey, searchKey, threshold, addonUrl) {
    ...
    if (fileType == FILE_TYPES.TORRENTS)
        results = files.map(result => toTorrent(apiKey, result, addonUrl))
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3, addonUrl) {
    return searchFiles(FILE_TYPES.TORRENTS, apiKey, searchKey, threshold, addonUrl)
}

async function searchDownloads(apiKey, searchKey = null, threshold = 0.3, addonUrl) {
    return searchFiles(FILE_TYPES.DOWNLOADS, apiKey, searchKey, threshold, addonUrl)
}
...
function toTorrent(apiKey, item, addonUrl) {
    const videos = item.files
        .filter(file => isVideo(file.short_name))
        .map((file) => {
            const url = `${addonUrl}/resolve/TorBox/${apiKey}/${item.id}/${file.id}`
