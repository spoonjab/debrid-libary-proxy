...
.manifest-url-wrap input {
	flex: 1;
}

.manifest-url-wrap .spinner-overlay {
	display: none;
	position: absolute;
	inset: 0;
...
.manifest-url-wrap .spinner-overlay.active {
	display: flex;
}
...
function landingTemplate(manifest, config) {
	...
	<div style="display:flex;align-items:center;gap:1.5vh;margin:2vh 0">
		<div style="flex:1;height:1px;background:rgba(255,255,255,0.2)"></div>
		<span style="font-size:1.4vh;opacity:0.5;white-space:nowrap;letter-spacing:0.08em;text-transform:uppercase">auto-generated</span>
		<div style="flex:1;height:1px;background:rgba(255,255,255,0.2)"></div>
	</div>

	<div id="manifestUrlBox">
		<div class="form-element">
			<div class="label-to-top">Stremio Plugin — Manifest URL</div>
			<div style="display:flex;gap:1vh">
				<div class="manifest-url-wrap">
					<input type="text" id="manifestUrl" readonly style="cursor:pointer;padding-left:15px" onclick="this.select()" placeholder="Fill in your Debrid Provider and API Key above">
					<div class="spinner-overlay" id="manifestSpinner"><div class="spinner"></div></div>
				</div>
				<button type="button" id="copyBtn" onclick="copyManifestUrl()" style="margin:0;white-space:nowrap">Copy</button>
			</div>
			<p style="margin-top:1vh;font-size:1.5vh;opacity:0.7">Copy the URL above and paste it into the <strong>Add addon</strong> box at <a href="https://web.stremio.com/#/addons" target="_blank">web.stremio.com/#/addons</a> or in the Stremio desktop app under <strong>Settings → Addons → Add addon</strong>.</p>
		</div>
	</div>
	...
	const spinner = document.getElementById('manifestSpinner')

	const showSpinnerThenUpdate = () => {
		...
		spinnerTimer = setTimeout(() => {
			...
			if (isValidConfig(config)) {
				input.value = buildManifestUrl(config)
				input.placeholder = ''
			} else {
				input.value = ''
				input.placeholder = 'Fill in your Debrid Provider and API Key above'
			}
		}, 500)
	}
	...
	['DebridApiKey', 'MediaFlowUrl', 'MediaFlowPassword'].forEach(id => {
		document.getElementById(id).addEventListener('input', showSpinnerThenUpdate)
	})
