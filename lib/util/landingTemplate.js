...
function landingTemplate(manifest, config) {
	...
	<form class="pure-form" id="mainForm">
		...
		<div class="form-element">
			<div class="label-to-top">MediaFlow Proxy URL <span style="font-weight:normal;opacity:0.7">(optional — routes streams through your proxy so Real-Debrid only sees your MediaFlow IP)</span></div>
			<input type="text" id="MediaFlowUrl" name="MediaFlowUrl" class="full-width" placeholder="http://your-mediaflow-server:8888">
		</div>
		<div class="form-element">
			<div class="label-to-top">MediaFlow API Password <span style="font-weight:normal;opacity:0.7">(optional)</span></div>
	...
	<div class="form-element" id="manifestUrlBox" style="display:none">
		<div class="label-to-top">Manifest URL — paste this into Stremio's addon search box</div>
	...
	const updateLink = () => {
		...
		if (isValidConfig(config)) {
			...
			installLink.href = 'stremio://' + window.location.host + '/' + encodeURIComponent(JSON.stringify(
				Object.fromEntries(Object.entries(config).filter(([k,v]) => v !== '' && v !== false))
			)) + '/manifest.json'
