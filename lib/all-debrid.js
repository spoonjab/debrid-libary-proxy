...
async function getTorrentDetails(apiKey, id, addonUrl) {
    const AD = new AllDebridClient(apiKey)

    return await AD.magnet.status(id)
        .then(res => toTorrentDetails(apiKey, res.data.magnets, addonUrl))
        .catch(err => handleError(err))
}

async function toTorrentDetails(apiKey, item, addonUrl) {
    const videos = item.links
        .filter(file => isVideo(file.filename))
        .map((file, index) => {
            const hostUrl = file.link
            const url = `${addonUrl}/resolve/AllDebrid/${apiKey}/${item.id}/${encode(hostUrl)}`
